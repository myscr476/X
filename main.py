...
while True:
    try:
        command = input(f"\033[93m~ $\033[0m").strip()
    except EOFError:
        print("Exiting...")
        break
    ...
    if command in aliases:
        ...
        if xstatus == "NO":
            ...
            if sub == "Starts with new code":
                ...
                if stepcode.startswith("ID"):
                    print("Checking id...")
                    time.sleep(random.randint(2, 5))
                    print("Invalid ID: No ID in X#Editor.")
                else:
                    print("Please start with 'ID' to check your ID.")
        ...
        try:
            cdirectory = command[len("cd"):].strip()
            dangerous_paths = ["/", "/storage", "/storage/emulated"]
            if cdirectory == "$HOME":
                os.chdir("/storage/emulated/0/X#init" if os.name != 'nt' else "C:\\Users")
            elif os.path.abspath(cdirectory) in [os.path.abspath(p) for p in dangerous_paths]:
                print("Access denied.")
                continue
            elif not os.path.exists(cdirectory):
                print("Path not found.")
            elif os.path.exists(cdirectory):
                os.chdir(cdirectory)
        except BaseException as e:
            print(f"{type(e).__name__}: {e}")
