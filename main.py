...
username = "local"
cdirectory = ("/storage/emulated/0/X#init" if "android" in platform.platform().lower()
    else r"C:\Users" if os.name == "nt"
    else "/home" if platform.system() == "Linux" and not ("ANDROID_ROOT" in os.environ or "ANDROID_DATA" in os.environ)
    else "~/" if os.name == "darwin" else None)
default_home = ("/storage/emulated/0/X#init" if "android" in platform.platform().lower()
    else r"C:\Users" if os.name == "nt"
    else "/home" if platform.system() == "Linux" and not ("ANDROID_ROOT" in os.environ or "ANDROID_DATA" in os.environ)
    else "~/" if os.name == "darwin" else None)
...
filename = "main.py"
if os.name == "posix":
	os.makedirs("/sdcard/X#init/homefolder", exist_ok=True)
else:
	pass	
if os.name == 'nt':
	pass
...
while True:
    try:
        ...
        if prompt.startswith("[chroot]"):
                ...
                prompt_path = "~"
        elif "android" in platform.platform() and cdirectory.startswith("/sdcard"):
        	prompt_path = "/storage/emulated/0"
        ...
        if command.strip() == "!!!":
        	if history:
        		...
        		print("There is no two previous command to run.")
        		continue
    ...
    if not command:
        ...
        while True:
            ...
            if subcommand == "%EXIT":
                ...
                print("%help[%EXIT,%HELP,%CHANGEUSER")
            elif subcommand == "%CHANGEUSER":
            	print("%changeuser[", end="")
        ...
        if xstatus == "NO":
            print("The program 'xeditor' is not installed. Install by:\n pkg install xeditor")
        else:
...
'''
        ...
        try:
            ...
            home_path = ("/storage/emulated/0/X#init" if "android" in platform.platform().lower()
    else r"C:\Users" if os.name == "nt"
    else "/home" if platform.system() == "Linux" and not ("ANDROID_ROOT" in os.environ or "ANDROID_DATA" in os.environ)
    else "~/" if os.name == "darwin" else None)
            ...
            os.chdir(target)
            cdirectory = target
            print("Succesfully changing directory!")
     ...
     cd /sdcard"""  
        ...
        if len(parts) != 2:
            ...
            errdesc = {
                ...
                "1": "EOF is called when program is running.",
        ...
        total_commands = 0
        with open(sys.argv[0], "r") as line:
            for lines in line:
                linesin = f.read()
        ...
        print(f"║ Lines of Code : {len(linesin)} ║")
        ...
        facts = [
            ...
            "Fun Fact: Cats have 9 health.",
            "Fun Fact: Observable Universe is that can we look for now."
        ]
        ...
        elif os.path.exists(cmdname) or os.path.isfile(cmdname) or os.path.isdir(cmdname):
        	print(f"{cmdname}: {os.path.abspath(cmdname)}")
        else:
...
"""
        ...
        try:
            new_home = ("/storage/emulated/0/X#init" if "android" in platform.platform().lower()
    else r"C:\Users" if os.name == "nt"
    else "/home" if platform.system() == "Linux" and not ("ANDROID_ROOT" in os.environ or "ANDROID_DATA" in os.environ)
    else "~" if os.name == "darwin" else None)
            os.chdir(new_home)
            cdirectory = new_home
            default_home = new_home
            env_vars["HOME"] = new_home
            prompt = "$"
      ...
      if not say:
    	...
    	utilscmd = args.strip()
    	if not utilscmd:
    		print(f"Welcome to package Utils.\nThis is the package that controled by lv.\n\nPackage utils is control all built-in command like {','.join(commands)} and another package.\n\n\n[usage]: utils --version\tutils --osutils")
    	elif utilscmd == "--version":
    		print("2.1")
    	elif utilscmd == "--osutils":
        ...
        if not text:
            ...
            sys.stdout.write(text.encode("utf-8").decode("unicode_escape"))
            sys.stdout.flush()
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"xsh: {r.status_code}")
        except Exception as e:
            print("Failed to fetch IP info:", e)
            
    elif command == "abcd":
      text = args.strip()
      print(text, "Argh, why you can get all the easter egg?!")
      time.sleep(0.3)
      print("Now...")
      time.sleep(0.9)
      print("GET OUT!")
      ...
      if text == "Stupid.":
          ...
          while True:
          	...
          	print("HAHAHA")
          	print("Kaboom!")
          	print("You just unlocked nothing! Hahaha!")
          	print("Roomie Corp.")
          	print("Are you want to give up? No? Okay! Hahahahaha!!!")
          	print("nah bro.")
          	
    elif command == "vault":
        print("You shouldn't be here...")
        while True:
          		vaulted = input("Vault of Secrets: this aint GD bruh.  ")
          		if vaulted == "break":
          			break
          			
    elif command == "nahigiveup":
          print("Well done...")
          print("Now i control this terminal.")
          while True:
          		print("Blah blah blah")
          		print("bleh\nsigma sigma boy")
          		print("WtHiSThAtNaN")
          		
    elif command == "ourinspirat":
      print("I make this terminal with no teams, just me.\nThis project created to make a terminal, that easy to read and simple.\nThis project is created by Athaya, or Roomie. He is born in Indonesia, and he is so young for now.\nInspiration from: Python Interpreter. Thanks for Pydroid 3 for make IDE that make me interested to Technology! I like playing Geometry Dash, and if you want to visit all my level, just search: Roomie12, thx for use my terminal! Bye!\n\n\n\nAthaya\t\t\t26 January, 2025")
      
      
    elif command == "na":
      nah = args.strip()
      print(f"N/A? what you mean? {nah}? Bro... Are you kidding me? Easter Egg Island? bruh BHAAHAHAHAHAHHA... (cough) okay okay i not kidding anymore. Now... Easter Egg Island? Okay okay... i will make the 'Easter Egg Island' file soon...")
    
    elif command == "zsh":
       print("This is not ZSH... Cool. You fooled me. NOW... GET OUT!!")
       print("BRAAAKKK!!!")
        ...
        if not args:
            print("Usage: kill <pid | process_name>")
            continue
        ...
        if args.isdigit():
            ...
            for p in psutil.process_iter(['pid', 'name']):
                try:
                    if args.lower() in p.info['name'].lower():
                        p.kill()
                        print(f"Killed {p.info['name']} PID {p.info['pid']}")
                        found = True
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    pass
        ...
        # S AI
        closest = difflib.get_close_matches(command, commands, n=10, cutoff=0.3)
        if closest:
            print("sai: did you mean::")
            for cmd in closest:
                print(f" - {cmd}")
        else:
